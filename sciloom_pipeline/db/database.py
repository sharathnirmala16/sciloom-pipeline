...
# Create engine with thread-safe settings and lock timeout for SQLite
...
def set_sqlite_pragma(dbapi_connection, connection_record):
    """Enables WAL mode, conditional foreign keys, and registers compatibility fallback functions on SQLite connections."""
    cursor = dbapi_connection.cursor()
    cursor.execute("PRAGMA journal_mode=WAL;")

    # Disable foreign keys during Alembic migrations to prevent SQLite cascade delete bugs on batch table alterations
    if os.environ.get("ALEMBIC_MIGRATION") == "true":
...
def init_db() -> None:
    ...
    try:
        ...
        if "ALEMBIC_MIGRATION" in os.environ:
            del os.environ["ALEMBIC_MIGRATION"]
