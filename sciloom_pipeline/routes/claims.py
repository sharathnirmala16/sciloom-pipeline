...
async def get_claims(job_id: str, db: Session = Depends(get_db)):
    ...
    return await job_service.get_claims_for_job(job_id)
...
async def sync_claims(job_id: str, request: ClaimsSyncRequest, db: Session = Depends(get_db)):
    """Synchronizes (adds, updates, removes) claims for a specific job in .sciloom/CLAIMS.json."""
    job_record = db.query(models.Job).filter(models.Job.id == job_id).first()
    if not job_record:
        raise HTTPException(status_code=404, detail=f"Job {job_id} not found.")

    incoming_claims = request.claims
    existing_claims = await job_service.get_claims_for_job(job_id)
    existing_map = {c["id"]: c for c in existing_claims if "id" in c}
    ...
    for idx, claim_data in enumerate(incoming_claims):
        ...
        if claim_id in existing_map:
            ...
            existing_claim["claimText"] = claim_text
            ...
            existing_claim["replicationError"] = claim_data.replication_error
            existing_claim["userInstructions"] = claim_data.user_instructions
            existing_claim["userScreenshots"] = claim_data.user_screenshots or []
            updated_claims.append(existing_claim)
        else:
            # Create new
            new_claim = {
                "id": claim_id,
                ...
                "replicationError": claim_data.replication_error,
                "userInstructions": claim_data.user_instructions,
                "userScreenshots": claim_data.user_screenshots or [],
                "createdAt": datetime.now().isoformat()
            }
            updated_claims.append(new_claim)

    await job_service.save_claims_to_file(job_id, updated_claims)
