#!/usr/bin/env python3
"""
Replication script for CLAIM-005:
"Mean feather cort concentrations in Midway samples were not statistically
 significantly different from Kure samples."
Expected: p = 0.18
"""
import pandas as pd
import numpy as np
...
def main():
    ...
    for i, (g1, g2) in enumerate(all_comps):
        if (g1 == 'Midway' and g2 == 'Kure') or (g1 == 'Kure' and g2 == 'Midway'):
            p_adj = p_corrected[i]
            break
    
    print(f"\nMidway vs Kure: adjusted p = {p_adj:.4f}")
    
    expected_p = 0.18
    p_match = abs(p_adj - expected_p) < 0.01
    
    if p_match:
        print("\n✓ CLAIM-005 REPLICATED SUCCESSFULLY")
        print(f"  p = {p_adj:.2f} (expected p = {expected_p})")
        
        with open(CLAIMS_PATH, 'r') as f:
            claims = json.load(f)
        for claim in claims:
            if claim['id'] == 'CLAIM-005':
